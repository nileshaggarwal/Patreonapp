	...
	User.findOne({ _id: loggedInAs }, (er, user) => {
		...
		patreonOAuthClient
			...
			.then(function (tokensResponse) {
				rf = tokensResponse.refresh_token;
...
exports.loginButtonClicked = (req, res) => {
	res.redirect(
		`https://www.patreon.com/oauth2/authorize?response_type=code&client_id=${process.env.CLIENT_ID}&redirect_uri=${redirectURL}&state=${req.app.locals._id}`
	);
};
